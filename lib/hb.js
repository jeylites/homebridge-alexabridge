...
function parseHbtoHA_Bridge(host, port, hapname, hb) {
    ...
    for (var accessory in hb.accessories) {
        ...
        if (onUrl) {
            ...
            addHABridgeDevice("http://localhost:8080", JSON.stringify(device));
...
function addHABridgeDevice(url, device) {
    ...
    request({
        ...
        if (err || response.statusCode != 200) {
            log("HA Bridge Add Device failed %s:%s", url, err);
            ...
            try {
                ...
                //                log.error(ex);
                log("HA Bridge Add Device Failed %s:%s", url, response.statusCode, response.statusMessage);
                log("HA Bridge Add Device Failed %s:%s", url, response.body);
